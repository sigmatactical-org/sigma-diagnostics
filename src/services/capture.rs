...
#[cfg(target_os = "linux")]
pub fn stop_capture(state: &AppState) -> Result<String, String> {
    use std::sync::mpsc;
...
#[cfg(not(target_os = "linux"))]
pub fn stop_capture(_state: &AppState) -> Result<String, String> {
    Err("SocketCAN is only available on Linux".to_string())
}

/// Check if a capture is currently running.
pub fn is_capture_running(state: &AppState) -> bool {
    #[cfg(target_os = "linux")]
    {
